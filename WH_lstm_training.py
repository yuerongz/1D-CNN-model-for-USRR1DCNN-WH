...
def training(work_dir, saving_tag, batch_sz, structure_szs, seq_h, rl_group, epoch_len=50, learning_rt=0.001):
    ...
    train_loader, test_loader = data_loaders.data_splitting([data_loaders.input_tensor_prep(), data_loaders.rl_dataset_prep()
    ...
    val_losses = []
    eval_val_losses = []
    model = LSTMModel(structure_szs).to(device)
    model.float()
    loss_function = nn.MSELoss()
    eval_loss_function = nn.L1Loss()
    optimizer = optim.Adam(model.parameters(), lr=learning_rt)#, weight_decay=0.00001)
    ...
    for epoch in range(epoch_len):
        ...
        if (np.mean(val_losses[-len(test_loader):]) <= 0.0015):     # RMSE = 3.873cm; early stopping
            break
...
if __name__ == '__main__':
    ...
    seq_h = 96
    rl_size = 50    # default
    model_structure = [33, 25, 64, 64, rl_size]   # input_dim, conv2_channel_out_dim, hidden 1-2, rv_no
    biis_n = lambda input_n, lstm_n: 4*input_n*lstm_n/(input_n + 4*lstm_n+1)
    print('BIIS layer number good? ', biis_n(model_structure[0], model_structure[2]) > model_structure[1])
