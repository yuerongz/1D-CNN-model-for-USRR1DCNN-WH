from base_func import pytorch_cuda_check
...
def load_saved_convo_model(model_file, epoch_needed=False):
    device = pytorch_cuda_check()
    checkpoint = torch.load(model_file)
    model = ConvoModel2RLs(checkpoint['model_structure'], checkpoint['seq_h']).to(device)
...
def speed_test(work_dir):
    ...
    for file_name in file_ls:
        model_specs = load_saved_convo_model(file_name, True)
        model = model_specs[0]
        model.eval()
        with torch.no_grad():
            sttm = time.time()
    ...
    print(np.sum(np.array(pred_time)), time.time() - sttm0)
    return 0


def validation_preds(work_dir):
    import glob
    from gdal_func import gdal_writetiff, gdal_asarray
    ...
    # pred_depths1 = gdal_asarray(f"{work_dir}final_depth_predictions_unfilled.tif")    # get raw prediction
